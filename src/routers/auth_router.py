...
async def reset_password_confirm(uidb64: str, token: str):
    ...
    if verified:
        return {"message": True, "uidb64": uidb64, "token": token}
    raise TOKEN_EXCEPTION


@auth_router.post('/set-new-password', status_code=status.HTTP_202_ACCEPTED)
async def register(data: ResetPasswordModel):
    req = data.dict()
    (verified, payload) = verify_signed_token(req['token'].encode(), req['uidb64'])
    if verified:
        user = await User.get(id=payload['id'])
        if user:
